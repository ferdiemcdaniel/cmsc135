...
class LearningSwitch(api.Entity):
    ...
    def handle_link_down(self, port):
        ...
        """
        self.routingTable.pop(self.routingTable.keys()[self.routingTable.values().index(port)])
        pass
    ...
    def handle_rx(self, packet, in_port):
        ...
        if packet.dst not in self.routingTable:
            ...
            if self.routingTable[packet.dst] == in_port:
                return
            else:
                self.send(packet, self.routingTable[packet.dst], flood=False)
